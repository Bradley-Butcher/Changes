use super::{App, CommentInputState, DOUBLE_CLICK_MS, GapExpandResult, SCROLL_SPEED};
...
pub fn handle_mouse(
    ...
    match mouse.kind {
        MouseEventKind::ScrollUp => {
            app.scroll_active_viewport(-(SCROLL_SPEED as isize), viewport);
            true
        }
        MouseEventKind::ScrollDown => {
            app.scroll_active_viewport(SCROLL_SPEED as isize, viewport);
        ...
        MouseEventKind::Down(MouseButton::Left) => {
            ...
            if is_double_click && click_row >= screen.content_y {
                let content_row = (click_row as usize).saturating_sub(screen.content_y as usize)
            ...
            // Status bar badges
            if click_row == screen.status_bar_row {
                let (ms, me) = screen.mode_badge_pos;
                let (vs, ve) = screen.view_badge_pos;
            ...
            // Tab bar (rows 0-2)
            if click_row <= 2 {
                for (i, &(start, end)) in screen.tab_positions.iter().enumerate() {
                ...
                .and_then(|layout| layout.expand_gap_at_row(content_row))
            {
                if let Some(req) = app.start_expand_gap(file_idx, gap_idx) {
                    ...
                    std::thread::spawn(move || {
                        let _ = tx.blocking_send(req.execute());
                    });
                }
                return true;
            }
        ...
        MouseEventKind::Down(MouseButton::Right) => {
            ...
            if mouse.row < screen.content_y {
                return false;
            }
            let content_row = (mouse.row as usize).saturating_sub(screen.content_y as usize)
                + app.current_scroll_offset();
